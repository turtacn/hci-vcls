...
hci-vcls 项目源于对真实 HCI 生产环境中控制平面失效问题的深度分析。现有HCI 集群将 ZooKeeper（ZK）、pmxcfs（CFS）、MySQL 和 HAService 紧密耦合于同一控制平面，形成了一个脆弱的共同命运组合：任意组件进入不健康状态，整个 HA 调度链路即告中断。
...
import (
    "context"
    "sangfor.com/ai-hci/hci-vcls/pkg/fdm"
)
...
import (
    "context"
    "time"
    "sangfor.com/ai-hci/hci-vcls/pkg/cache"
    "sangfor.com/ai-hci/hci-vcls/pkg/fdm"
)
...
qm 适配器封装了对 QMP 协议命令行工具的调用，提供幂等性保证和结构化错误处理。
...
import (
    ...
    "sangfor.com/ai-hci/hci-vcls/internal/logger"
    "sangfor.com/ai-hci/hci-vcls/pkg/metrics"
)
...
type CFSMetaSource struct {
    // HCIConfigBase 是 配置目录根路径，通常为 /etc/sf。
    HCIConfigBase string
